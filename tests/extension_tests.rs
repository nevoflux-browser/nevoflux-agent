...
fn test_extension_manifest_exists() {
    ...
    assert!(manifest_path.exists(), "Extension manifest should exist at {:?}", manifest_path);
}
...
fn test_extension_manifest_valid_json() {
    ...
    let content = std::fs::read_to_string(&manifest_path)
        .expect("Failed to read extension manifest");
...
fn test_native_host_template_exists() {
    ...
    assert!(template_path.exists(), "Native host template should exist at {:?}", template_path);
}

#[test]
fn test_extension_readme_exists() {
    let readme_path = project_root().join("extension/README.md");
    assert!(readme_path.exists(), "Extension README should exist at {:?}", readme_path);
}

#[test]
fn test_extension_background_js_exists() {
    let bg_path = project_root().join("extension/background.js");
    assert!(bg_path.exists(), "Background script should exist at {:?}", bg_path);
}
