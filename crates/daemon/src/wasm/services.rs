...
impl HostServices {
    ...
    /// Ensure a skill is resolvable, lazily rescanning the skill registry
    /// **once** on a miss.
    ///
    /// The registry is a snapshot taken at daemon startup. A skill written
    /// during the current session — e.g. one just generated by skill-creator
    /// from a recording — is absent from that snapshot, so a `/skillname`
    /// invocation or a `skill_load` call would report "not found" until a
    /// restart. Calling this before a lookup rescans the configured skill
    /// directories so the new skill activates immediately. No-op when the skill
    /// is already present (the common case), so it stays off the hot path.
    ///
    /// Double-checked under the write lock: if a concurrent lookup already
    /// reloaded between our read miss and acquiring the write lock, we skip the
    /// redundant rescan. Async (tokio lock) variant for the ACP / server paths.
    pub async fn ensure_skill_or_reload(&self, name: &str) {
        if self.skills.read().await.get(name).is_some() {
            return;
        }
        let mut registry = self.skills.write().await;
        if registry.get(name).is_none() {
            if let Err(e) = registry.reload() {
                tracing::warn!("ensure_skill_or_reload: skill registry reload failed: {e}");
            }
        }
    }

    /// Blocking variant of [`Self::ensure_skill_or_reload`] for the synchronous
    /// WASM host-function path (direct-API providers).
    pub fn ensure_skill_or_reload_blocking(&self, name: &str) {
        if self.skills.blocking_read().get(name).is_some() {
            return;
        }
        let mut registry = self.skills.blocking_write();
        if registry.get(name).is_none() {
            if let Err(e) = registry.reload() {
                tracing::warn!(
                    "ensure_skill_or_reload_blocking: skill registry reload failed: {e}"
                );
            }
        }
    }
