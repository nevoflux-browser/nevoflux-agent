...
impl HostFunctions for DaemonHostFunctions {
    ...
    fn skill_load(&self, name: &str) -> HostResult<String> {
        ...
        // Load skills from filesystem if registry is empty (lazy loading)
        self.ensure_skills_loaded(services);
        // Lazily rescan once if `name` isn't in the current snapshot (e.g. a
        // skill just generated by skill-creator), so it loads without a restart.
        services.ensure_skill_or_reload_blocking(name);
    ...
    fn tool_call_dynamic(
        ...
        // Intercept Record & Replay tools (start_recording / stop_recording).
        //
        // Daemon-orchestrated like browser_input: they touch the
        // RecordingCollector + recordings_dir on BrowserContext and dispatch a
        // recording_start/recording_stop BrowserRequest, so the MCP fallthrough
        // would fail with "No server provides tool". The ACP/MCP-bridge path
        // mirrors this in `mcp_tool_executor::execute_mcp_tool`; both delegate to
        // the same `agent::tools::dispatch_recording_tool` helper.
        if tool_name == "start_recording" || tool_name == "stop_recording" {
            let services = self.services.as_ref().ok_or_else(|| HostError {
                code: 1,
                message: "Services not available".into(),
            })?;
