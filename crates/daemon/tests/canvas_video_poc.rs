...
/// Length-prefixed JSON proxy client for the PoC gate. Intentionally minimal
/// — it doesn't implement heartbeats, reconnection, or the full proxy surface
/// that the production bridge offers. It only needs to drive two renders and
/// listen on the chat channel for responses and EventBus deliveries.
mod tcp_client {
