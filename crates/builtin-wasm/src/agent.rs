...
{}"#,
    ...
    fn execute_tool(&self, tool_call: &ToolCall) -> HostResult<ToolResult> {
        ...
        let content = match normalized_name {
            ...
            "loop_scratchpad_get" => self.host.tool_loop_scratchpad_get(
                &serde_json::to_string(&tool_call.arguments).unwrap_or_default(),
            )?,
...
scope=\"live_folder\"). Omit to include all spaces/folders for the chosen scope."
            ...
            ToolDefinition {
                ...
                input_schema: serde_json::json!({
                    "type": "object",
                    "properties": {
                        "text":           { "type": "string", "description": "Text to speak. Pass the whole passage; it is cut on sentence boundaries internally. Max 327680 chars." },
            ...
            ToolDefinition {
                ...
                input_schema: serde_json::json!({
                    ...
                    "properties": {
                        ...
                        "wrapped_skill": {
                            "type": "string",
                            "description": "JSON-stringified `{name, args}` blob — XOR with prompt_text. Call JSON.stringify before passing. Sent as a string (not an object) because OpenAI Responses API strict-mode rejects bare `{type:object}` subschemas."
                        },
                        "mode": {
                            "type": "string",
